...
def noise_rw(x,snr):    #增加噪声
    x = x.transpose((1,0))  #tensorflow转pytorch
    snr1 = 10 ** (snr / 10.0)  #snr不同，噪声也不同
    xpower = np.sum(x ** 2,axis=0) / len(x)
    npower = xpower / snr1
    noise = np.random.normal(0, np.sqrt(npower), x.shape)  #随机生成正态分布，加了噪音更能评估模型的泛化能力
    noise_data = x+noise
    noise_data = noise_data.transpose((1,0))
    return noise_data
...
def plot_with_labels(data):
    ...
    for i, label in enumerate(range(10)):
        ...
        #add data points 
        plt.scatter(x=data.loc[data['label']==label, 'x'],  # 画散点图
